...
1. Read the occupancy load factors from `~/.claude/skills/occupancy-calculator/data/occupancy-load-factors.json`
2. Read the use group classifications from `~/.claude/skills/occupancy-calculator/data/use-groups.json`
3. Check if an `occupancy.json` exists in the current directory — if so, load it as the current calculation state
4. Check if a `program.json` exists in the current directory — if so, note it and offer to calculate occupancy from the workplace program's room schedule
